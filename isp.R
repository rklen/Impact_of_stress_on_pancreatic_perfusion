...
studyNumbers<-c()
for(i in c(1:19,21:27,29:75,78:130)){
  if(i>99){
    studyNumbers<-c(studyNumbers,paste('koveri0',i,sep=''))
  }else{
    if(i>9){
      studyNumbers<-c(studyNumbers,paste('koveri00',i,sep=''))
    }else{
      studyNumbers<-c(studyNumbers,paste('koveri000',i,sep=''))
    }
  }
}
...
write.csv(df1,file=paste('isp_new1.csv',sep=''),row.names=F)

df<-matrix(data=NA,nrow=length(studyNumbers),ncol=26)
df[,1]<-studyNumbers
...
df[,4]<-c(90,94,79,85,73,82,81,108,63,92,64,117,99,59,89,100,103,
         ...
         127,76,98,98,77,85,68)[c(1:19,21:27,29:75,78:130)]
...
df1<-read.csv('C:/Users/Oona/Documents/Tpc/isp/isp_new1.csv')
df[,21]<-abs(df1[,1])*60
df[,22]<-abs(df1[,2])*60
df[,23]<-abs(df1[,1]/df1[,2])
df[,24]<-1/(1+exp(-df1[,3]))
df[,25]<-df1[,4]
df[,26]<-(as.numeric(df[,15])-as.numeric(df[,9]))/as.numeric(df[,9])

df<-df[c(1:76,78:92,94:107,110:125),]
plot(1:121,as.numeric(df[,9]))
plot(1:121,as.numeric(df[,15]))
plot(1:121,as.numeric(df[,20]))
...
df0<-df0[c(1:76,78:92,94:107,110:125),]
df1<-df1[c(1:76,78:92,94:107,110:125),]
df2<-df2[c(1:76,78:92,94:107,110:125),]
df3<-df3[c(1:76,78:92,94:107,110:125),]
